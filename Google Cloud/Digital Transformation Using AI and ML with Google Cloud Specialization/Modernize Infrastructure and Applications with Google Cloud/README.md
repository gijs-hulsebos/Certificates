<p align="center">
  <img src="https://raw.githubusercontent.com/gijs-hulsebos/gijs-hulsebos/main/GijsHulsebos.banner.png" alt="Gijs Hulsebos AI Automation" width="100%" />
</p>

<p align="center">
  <a href="https://gijshulsebos.com">
    <img src="https://img.shields.io/badge/gijshulsebos.com-121212?style=for-the-badge&logo=googlechrome&logoColor=white" alt="Website">
  </a>
  &nbsp;
  <a href="https://www.linkedin.com/in/gijs-hulsebos">
    <img src="https://img.shields.io/badge/LinkedIn-121212?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn">
  </a>
</p>

---

### 🏫 Google Cloud: Modernize Infrastructure and Applications with Google Cloud

Explores cloud migration, infrastructure choices, cloud-native application development, and APIs as modernization tools.

Official course information: [Coursera course page](https://www.coursera.org/learn/google-cloud-product-fundamentals/).

---

### 🎯 Course Overview

This directory documents the verified course credential and the official module structure published by Coursera. Module notes are concise, factual portfolio summaries rather than reproductions of protected course materials.

---

### 📚 Modules

| Module | Topics |
| :--- | :--- |
| [Module 1: Fundamentals of modernization](./Module%201%20-%20Fundamentals%20of%20modernization.md) | Migration terminology; elastic cloud models; financial and operational impact |
| [Module 2: Modernizing infrastructure in the cloud](./Module%202%20-%20Modernizing%20infrastructure%20in%20the%20cloud.md) | Virtual machines, containers, and serverless; hybrid and multicloud management |
| [Module 3: Modernizing apps in the cloud](./Module%203%20-%20Modernizing%20apps%20in%20the%20cloud.md) | Cloud-native development; migration paths; APIs and Apigee |
| [Module 4: Course summary](./Module%204%20-%20Course%20summary.md) | Review of infrastructure and application modernization concepts |

---

### 🧠 Key Learnings

- Explain the course's core concepts using the issuer's terminology.
- Connect the module topics to practical decisions and workflows.
- Evaluate relevant AI or digital-work practices with attention to their capabilities and limitations.

---

### 🛠 Skills

- Cloud migration
- Cloud infrastructure
- Cloud-native applications
- APIs

---

### ✅ Verified Credential

**Completion Date:** March 28, 2026

👉 **[View Course Certificate](./Modernize%20Infrastructure%20and%20Applications%20with%20Google%20Cloud%20-%20Coursera%209D8T3ZSD484R.pdf)**
---

### 🎓 Part of

This course is part of the **Digital Transformation Using AI and ML with Google Cloud Specialization**.

👉 [View the full program](../README.md)

---

### 🌐 Let's Connect

* **Website:** [gijshulsebos.com](https://gijshulsebos.com)
* **LinkedIn:** [in/gijs-hulsebos](https://www.linkedin.com/in/gijs-hulsebos)

---

<p align="center">
  <img src="https://raw.githubusercontent.com/gijs-hulsebos/gijs-hulsebos/main/readme.md.banner.png" alt="Gijs Hulsebos AI Automation Footer" width="100%" />
</p>
